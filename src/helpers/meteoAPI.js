...
const getinfo = (day)=>{
    return { day: day.day, ...day.forecastTimeSlots.reduce(getMinMaxTemp,{tempMin:1000, tempMax:'',forecastWeather:''})};
};
...
export function getCurrentWeather(cityId){
    ...
    return axios(`${url}/weather?appid=${apiKey}&id=${cityId}`)
        .then(res => { return ({ 'currentTemp': Math.floor(res.data.main.temp -273.15),'currentIcon': res.data.weather[0].id, 'cityName':res.data.name  })})
        .catch((err) => {
...
export function getForecastWeather(cityId){
    ...
    return axios(`${url}/forecast?appid=${apiKey}&id=${cityId}`)
        .then(res => {
           ...
           const forecasts = Object.entries(res.data.list.reduce(groupByDay,{})).map((ele)=>{ return {day:ele[0],forecastTimeSlots:ele[1]}})
               .filter((day)=>day.forecastTimeSlots.length == 8 ).slice(0,3).map(getinfo).map(convertTemperatureInCelsius);
